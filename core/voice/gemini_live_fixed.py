...
Uses gemini-3.1-flash-live-preview (latest live model)

Supports two modes:
1. Direct connection to Gemini Live API (requires GEMINI_API_KEY)
2. Proxy connection via Indus Proxy Server (key stored server-side, like Zoya)
"""
...
GEMINI_LIVE_VOICES = {
    ...
    "Autonoe": "Female, soft",
    "Enceladus": "Male, gentle",
    "Iapetus": "Male, calm",
    "Umbriel": "Male, expressive",
    "Algieba": "Female, lively",
    "Despina": "Female, sweet",
    "Erinome": "Female, clear",
    "Algenib": "Male, smooth",
    "Rasalgethi": "Male, deep warm",
    "Laomedeia": "Female, melodic",
    "Achernar": "Male, soft",
    "Alnilam": "Male, rich",
    "Schedar": "Male, gentle",
    "Gacrux": "Male, warm",
    "Pulcherrima": "Female, elegant",
    "Achird": "Male, friendly",
    "Zubenelgenubi": "Male, conversational",
    "Vindemiatrix": "Female, crisp",
    "Sadachbia": "Female, bright",
    "Sadaltager": "Male, mellow",
    "Sulafat": "Female, smooth",
}
...
class GeminiLiveClient:
    ...
    1. Direct: Connects directly to Gemini Live API (requires GEMINI_API_KEY)
    2. Proxy: Connects via Indus Proxy Server (key stored server-side, like Zoya)
    """
...
def __init__(
        ...
        self.proxy_url = proxy_url or os.getenv("PROXY_URL")  # e.g., ws://localhost:3001/ws/live
    ...
    def start_audio_streams(self):
        ...
        # Output stream (speaker) - keep callback for playback
    ...
    async def _run_direct(self):
        ...
        # Use gemini-2.0-flash-exp (working model for Live API)
        ...
        # Task to send microphone audio - Zoya style
        async def send_loop(session):
            ...
            while self.is_running:
                try:
                    ...
                    # Zoya style: use session.send() with input dict
                    await session.send(
        ...
        async def receive_loop(session):
            async for response in session.receive():
                ...
                if server_content.turn_complete:
                    ...
                    # Could trigger end_of_turn here if needed
        ...
        while self.is_running and reconnect_count < max_reconnects:
            try:
                async with websockets.connect(
                    ...
                    print(f"[Gemini Live] Connected via proxy with voice: {self.voice}, persona: {self.persona}")
                    
                    # Task to send microphone audio to proxy
                    async def send_loop():
                        chunks_sent = 0
                        while self.is_running:
                            try:
                    ...
                    # Task to receive audio/text from proxy
                    async def receive_loop():
                        async for message in ws:
                            ...
                            try:
                                ...
                                if msg_type == "audio" and parsed.get("audio"):
                                    # Decode base64 audio and play
                                    audio_bytes = base64.b64decode(parsed["audio"])
                                    self.response_queue.put(audio_bytes)
                                
                                elif msg_type == "transcript" and parsed.get("text"):
                                    print(f"[Gemini Live] Text: {parsed['text']}")
                                
                                elif msg_type == "interrupted":
                                    print("[Gemini Live] Interrupted")
                                
                                elif msg_type == "toolCall":
                                    print(f"[Gemini Live] Tool call: {parsed.get('toolCall')}")
                                
                                elif msg_type == "error":
                                    print(f"[Gemini Live] Error: {parsed.get('message')}")
                                
                                elif msg_type == "turnComplete":
                                    pass
                                
                                elif msg_type == "sessionClosed":
                                    print(f"[Gemini Live] Session closed: {parsed.get('reason')}")
                                    return  # Exit receive loop to trigger reconnect
                                    
                            except Exception as e:
                                print(f"[Debug] Receive error: {e}")
                    
                    # Run both loops until one exits
                    ...
                    # If we're still running, it means session closed - attempt reconnect
                    if self.is_running:
                        reconnect_count += 1
                        print(f"[Gemini Live] Session ended, reconnecting... (attempt {reconnect_count}/{max_reconnects})")
                ...
                if self.is_running:
                    ...
                    if reconnect_count < max_reconnects:
                        ...
                        print(f"[Gemini Live] Max reconnection attempts reached")
                        break
...
def create_gemini_live_client(
    ...
    voice: str = "Aoede",
    persona: str = "zoya",
    proxy_url: Optional[str] = None,  # NEW: Proxy server URL
    **kwargs
) -> GeminiLiveClient:
