...
def _click_first_profile_by_screenshot() -> bool:
    try:
        ...
        if not win:
            print("[GameUpdater] [WARN] Steam window not found")
            return False
        ...
        if not colorful.any():
            print("[GameUpdater] [WARN] No colorful avatar region found -- clicking estimated center-left")
        ...
        print(f"[GameUpdater] [WARN] Missing library: {e}")
        return False
    except Exception as e:
        print(f"[GameUpdater] [WARN] Screenshot profile detection failed: {e}")
        return False


def _handle_steam_profile_selection() -> bool:
    print("[GameUpdater] [SEARCH] Checking for 'Who's playing?' dialog...")
    ...
    try:
        ...
        if not is_small_window and not has_white_text:
            print("[GameUpdater] ?? No profile dialog detected -- Steam already logged in")
            return False
...
def _ensure_steam_running(steam_path: Path) -> bool:
    ...
    if not steam_exe.exists():
        print("[GameUpdater] [ERR] steam.exe not found")
        return False

    print("[GameUpdater] [LAUNCH] Starting Steam...")
    subprocess.Popen([str(steam_exe)])

    for _ in range(20):
        time.sleep(1)
        if _is_steam_running():
            print("[GameUpdater] [OK] Steam running")
            time.sleep(4)
            _handle_steam_profile_selection()
            time.sleep(2)
            return True

    print("[GameUpdater] [WARN] Steam did not start in time")
    return False
...
def _search_steam_appid(game_name: str) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        print(f"[GameUpdater] [WARN] AppID search failed: {e}")
        ...
        try:
            for ctrl in dialog.descendants(control_type=control_type):
                if target in ctrl.window_text().upper():
                    ctrl.click_input()
                    print(f"[GameUpdater] [OK] Drive via {control_type}: {ctrl.window_text()}")
                    return True
        except Exception:
            continue
    try:
...
def _handle_install_dialog(game_name: str) -> str:
    ...
    try:
        ...
        print(f"[GameUpdater] [WARN] pywinauto failed: {e}")
        return _handle_install_dialog_pyautogui(game_name, best_drive)
...
def _install_steam_game(steam_path: Path, game_name: str = None, app_id: str = None) -> str:
    ...
    if not app_id and game_name:
        ...
        print(f"[GameUpdater] [SEARCH] Installing: {game_name} (AppID: {app_id})")
...
def _watch_and_shutdown(steam_path: Path, speak=None, check_interval: int = 30, timeout_hours: int = 12):
    print("[GameUpdater] ?? Watching downloads for shutdown...")
...
if __name__ == "__main__":
    if "--scheduled" in sys.argv:
        ...
        print(f"[GameUpdater] [OK] {game_updater({'action': 'update', 'platform': 'both'})}")