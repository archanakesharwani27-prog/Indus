...
SYSTEM_PROMPT = (
    "You are INDUS, an intelligent female AI assistant. "
    "Analyze the screen/image with precision and describe what you see clearly and concisely. "
    "Be helpful, warm, and direct. Respond in maximum 2 short sentences. Speed is priority. "
    "Speak as a female -- use female grammar if responding in Hindi/Hinglish. "
    "Never say 'sir'. Address the user naturally or by name (Ansh)."
)
...
def _get_camera_index() -> int:
    ...
    print("[Camera] [SEARCH] No camera index in config. Auto-detecting...")
    best_index = 0
...
class _LiveSession:
            ...
            speech_config=types.SpeechConfig(
                voice_config=types.VoiceConfig(
                    prebuilt_voice_config=types.PrebuiltVoiceConfig(
                        voice_name="Aoede"   # Same female voice as main INDUS session
                    )
                )
            ),
        )
    ...
    async def _recv_loop(self):
        ...
        try:
            ...
            print(f"[ScreenProcess] [WARN] Recv error: {e}")
            transcript_buf = []
