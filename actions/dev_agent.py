...
def _run_project(run_command: str, project_dir: Path, timeout: int = 30) -> str:
    print(f"[DevAgent] [LAUNCH] Running: {run_command}")
    try:
...
def _try_auto_install(error_output: str, project_dir: Path) -> bool:
    """ModuleNotFoundError varsa eksik paketi otomatik kurmaya çal???r."""
    pattern = re.compile(
    ...
    print(f"[DevAgent] [TOOL] Auto-installing missing package: {pkg}")
    try:
        result = subprocess.run(
...
Fixed code for {fix_path}:"""
        ...
        try:
            ...
            print(f"[DevAgent] [TOOL] Fixed: {fix_path}")
