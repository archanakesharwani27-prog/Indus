...
#   screen_debug ? Screenshot al, ekrandaki kodu/hatay? Gemini ile analiz et ve düzelt
        ...
        try:
            ...
            for model_name in GEMINI_CODING_MODELS:
                try:
                    ...
                    print(f"[CodeHelper] {model_name} error: {m_err}")
        except Exception as e:
            print(f"[CodeHelper] GenAI client error: {e}, falling back to OpenRouter/Groq...")

    try:
        from or_client import client as or_c
        return or_c.fast_chat(prompt, system=system, max_tokens=3000)
    except Exception as e:
        print(f"[CodeHelper] OpenRouter error: {e}")
        return ""
...
def _detect_intent(description: str, file_path: str, code: str) -> str:
    ...
    screen_kw = ["ekrandaki", "screen", "ekranda", "bu hatay?", "why am i getting",
                 "neden hata", "what's wrong", "ne yanl??", "screenshot", "görüntü"]
    if any(k in desc for k in screen_kw):
        return "screen_debug"

    # Optimize keywords
    optimize_kw = ["optimize", "refactor", "clean up", "improve", "temizle",
                   "iyile?tir", "daha iyi", "make it better", "h?zland?r"]
    ...
    if file_path:
        ...
        edit_kw  = ["edit", "update", "modify", "change", "add", "remove",
                    "refactor", "fix", "rename", "replace", "düzenle", "de?i?tir"]
        run_kw   = ["run", "execute", "launch", "start", "çal??t?r"]
    ...
    explain_kw = ["explain", "what does", "describe", "analyze", "aç?kla", "ne yap?yor"]
...
Code:"""
    ...
    raw_text = _call_coding_model(prompt, system=f"You are an expert {lang} developer. Output only clean code.")
    code     = _clean_code(raw_text)
...
Fixed code:"""
    ...
    raw_text = _call_coding_model(prompt, system="You are an expert software debugger. Return only fixed code.")
...
def _run_file(path: Path, args: list, timeout: int) -> str:
    if path.suffix.lower() == ".py":
        try:
            from core.code_sandbox import run_sandboxed_python
            code = path.read_text(encoding="utf-8", errors="replace")
            res = run_sandboxed_python(code, timeout=timeout, custom_cwd=path.parent, args=args)
            if res.get("security_blocked"):
                return f"[Security Block] {res.get('stderr')}"
            parts = []
            if res.get("stdout"): parts.append(f"Output:\n{res['stdout'].strip()}")
            if res.get("stderr"): parts.append(f"Stderr:\n{res['stderr'].strip()}")
...
def _build(description, language, output_path, args, timeout, speak=None, player=None) -> str:
    ...
    for attempt in range(1, MAX_BUILD_ATTEMPTS + 1):
        print(f"[Code] [RETRY] Attempt {attempt}/{MAX_BUILD_ATTEMPTS}")
        if player:
...
Updated code:"""
    ...
    raw_text = _call_coding_model(prompt, system="You are an expert code editor. Return only updated code.")
    edited   = _clean_code(raw_text)
...
Explanation:"""
    ...
    raw_text = _call_coding_model(prompt, system="You are an expert code explainer. Give clear concise explanations.")
    return raw_text.strip() if raw_text else "Could not explain code."
...
Optimized code:"""
    ...
    raw_text  = _call_coding_model(prompt, system=f"You are an expert {lang} optimizer. Return only optimized code.")
    optimized = _clean_code(raw_text)
    if not optimized:
